...
import mobpair.com.installtrack.mock.MockHttpStack;
import mobpair.com.installtrack.toolbox.BasicNetwork;
...
public class BasicNetworkTest {
    ...
    @Test public void socketTimeout() throws Exception {
        ...
        doThrow(new VolleyError()).when(mMockRetryPolicy).retry(ArgumentMatchers.any(VolleyError.class));
        try {
            httpNetwork.performRequest(request);
        } catch (VolleyError e) {
            // expected
        ...
        verify(mMockRetryPolicy).retry(ArgumentMatchers.any(TimeoutError.class));
    }
    ...
    @Test public void noConnection() throws Exception {
        ...
        verify(mMockRetryPolicy, never()).retry(ArgumentMatchers.any(VolleyError.class));
    }
    ...
    @Test public void unauthorized() throws Exception {
        ...
        verify(mMockRetryPolicy).retry(ArgumentMatchers.any(AuthFailureError.class));
    }
    ...
    @Test public void serverError_enableRetries() throws Exception {
        for (int i = 500; i <= 599; i++) {
            ...
            verify(mMockRetryPolicy).retry(ArgumentMatchers.any(ServerError.class));
            reset(mMockRetryPolicy);
        }
    }
